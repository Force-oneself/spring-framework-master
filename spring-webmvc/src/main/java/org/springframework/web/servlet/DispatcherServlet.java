...
public class DispatcherServlet extends FrameworkServlet {
	...
	protected void onRefresh(ApplicationContext context) {
		// 初始化九大组件，通过Spring容器提供Bean
		// 容器中没有时，如果有需要会通过加载DispatcherServlet.properties文件获取默认
	 ...
	 * <p>如果在 BeanFactory 中没有为这个命名空间定义给定名称的 bean，
	 * 则不提供多部分处理.
	 */
	 ...
	 * <p>如果在 BeanFactory 中没有为这个命名空间定义给定名称的 bean，
	 * 我们默认为 AcceptHeaderLocaleResolver.
	 */
	 ...
	 * <p>如果在 BeanFactory 中没有为这个命名空间定义 HandlerMapping bean，
	 ...
	 * <p>如果在 BeanFactory 中没有为这个命名空间定义 HandlerAdapter bean，
	 * 我们默认为 SimpleControllerHandlerAdapter。
	 */
	private void initHandlerAdapters(ApplicationContext context) {
		...
		// 如果没有找到其他适配器，则通过注册默认的 HandlerAdapter 来确保我们至少有一些 HandlerAdapter.
	 ...
	 * <p>默认实现使用“DispatcherServlet.properties”文件
	 * （与 DispatcherServlet 类在同一个包中）来确定类名。
	...
	protected <T> List<T> getDefaultStrategies(ApplicationContext context, Class<T> strategyInterface) {
		if (defaultStrategies == null) {
			try {
				// 从属性文件加载默认策略实现。这目前是严格内部的，不打算由应用程序开发人员定制.
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				// 确定当前请求的处理程序.
				// 获取handler处理器，返回的mappedHandler封装了handlers和interceptors
				...
				// 从 4.3 开始，我们也在处理从处理程序方法抛出的错误，使它们可用于 @ExceptionHandler 方法和其他场景。
			...
			if (asyncManager.isConcurrentHandlingStarted()) {
				// 而不是 postHandle 和 afterCompletion
				if (mappedHandler != null) {
					// 并发处理拦截器执行
	...
	/**
	 * 将请求转换为多部分请求，并使多部分解析器可用。 <p>如果没有设置多部分解析器，只需使用现有的请求.
	 *
	 * @param request current HTTP request
	...
	private void restoreAttributesAfterInclude(HttpServletRequest request, Map<?, ?> attributesSnapshot) {
		// 此处需要复制到单独的 Collection 中，以避免在删除属性时对 Enumeration 产生副作用.
