...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	 ...
	 * <p>委托 {@link #createWebApplicationContext} 实际创建上下文。可以在子类中覆盖。
	 *
	...
	protected WebApplicationContext initWebApplicationContext() {
			...
			if (wac instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					if (cwac.getParent() == null) {
						// 上下文实例是在没有显式父级的情况下注入的 -> 将根应用程序上下文（如果有；可能为 null）设置为父级
		...
		if (wac == null) {
			// 在构建时没有注入上下文实例 -> 查看是否已在 servlet 上下文中注册。
			// 如果存在，则假定已经设置了父上下文（如果有）并且用户已执行任何初始化，例如设置上下文 id
		...
		if (!this.refreshEventReceived) {
			// 上下文不是具有刷新支持的 ConfigurableApplicationContext
			// 或在构建时注入的上下文已经刷新 -> 此处手动触发初始 onRefresh.
	...
	/**
	 * ApplicationListener 端点仅从该 servlet 的 WebApplicationContext 接收事件，
	 * 委托给 FrameworkServlet 实例上的 {@code onApplicationEvent}.
	 */
